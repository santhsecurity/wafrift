#!/usr/bin/env bash
# waf-zoo/modsec-aws/wafrift-bench.sh
# Run wafrift bench-waf against the ModSecurity + AWS-CRS-emulation stack.
#
# Usage:
...
#   docker compose -f waf-zoo/modsec-aws/docker-compose.yml up -d
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"

TARGET_URL="http://127.0.0.1:18101"
RESULT_DIR="$REPO_ROOT/wafrift-bench/results"
TIMESTAMP="$(date -u +%Y%m%d-%H%M%S)"
OUTPUT="$RESULT_DIR/${TIMESTAMP}-modsec-aws.json"
