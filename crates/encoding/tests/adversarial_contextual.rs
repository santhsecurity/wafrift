...
fn null_byte_json_string_escaped_not_rejected() {
    ...
    let out = encode_in_context(b"\x00", Strategy::CaseAlternation, InjectionContext::JsonString).unwrap();
    assert_eq!(out, "\\u0000");
}
...
fn unicode_rtl_override_html_attribute() {
    let out = encode_in_context("\u{202e}evil".as_bytes(), Strategy::CaseAlternation, InjectionContext::HtmlAttribute).unwrap();
...
fn chinese_characters_json_string() {
    let out = encode_in_context("中文测试".as_bytes(), Strategy::CaseAlternation, InjectionContext::JsonString).unwrap();
...
fn emoji_json_string() {
    ...
    let out = encode_in_context("Hello 👋".as_bytes(), Strategy::CaseAlternation, InjectionContext::JsonString).unwrap();
    assert_eq!(out, "HeLlO 👋");
}
...
fn surrogate_pairs_json_string() {
    ...
    let out = encode_in_context("😀".as_bytes(), Strategy::CaseAlternation, InjectionContext::JsonString).unwrap();
    assert_eq!(out, "😀");
}
...
fn empty_payload_all_contexts() {
    for ctx in all_contexts() {
        ...
        assert!(result.is_ok(), "empty payload failed for {:?}: {:?}", ctx, result);
    }
}
...
fn header_value_crlf_injection_prevented() {
    ...
    let err = encode_in_context(b"value\r\nX-Injection: evil", Strategy::CaseAlternation, InjectionContext::HeaderValue).unwrap_err();
    assert!(err.to_string().contains("incompatible"));
}
...
fn multipart_field_crlf_injection_prevented() {
    ...
    let err = encode_in_context(b"field\r\n--boundary", Strategy::CaseAlternation, InjectionContext::MultipartField).unwrap_err();
    assert!(err.to_string().contains("incompatible"));
}
...
fn cookie_value_crlf_injection_prevented_indirectly() {
    ...
    let out = encode_in_context(b"a\r\nb", Strategy::CaseAlternation, InjectionContext::CookieValue).unwrap();
...
fn invalid_utf8_bytes_handled() {
    ...
    let err = encode_in_context(&payload, Strategy::CaseAlternation, InjectionContext::JsonString).unwrap_err();
...
fn overlong_utf8_sequence() {
    ...
    let err = encode_in_context(&payload, Strategy::CaseAlternation, InjectionContext::PlainBody).unwrap_err();
    assert!(err.to_string().contains("invalid"));
}
...
fn base64_in_json_string_produces_valid_json() {
    let out = encode_in_context(b"hello", Strategy::Base64Encode, InjectionContext::JsonString).unwrap();
...
fn html_entity_encode_in_xml_attribute_double_escapes() {
    ...
    let out = encode_in_context(b"<", Strategy::HtmlEntityEncode, InjectionContext::XmlAttribute).unwrap();
...
fn chunked_split_in_header_value_rejected() {
    ...
    let err = encode_in_context(b"payload", Strategy::ChunkedSplit, InjectionContext::HeaderValue).unwrap_err();
    assert!(err.to_string().contains("incompatible"));
}
...
fn every_strategy_produces_valid_plain_body() {
    ...
    for strategy in wafrift_encoding::all_strategies() {
        ...
        assert!(!out.is_empty(), "strategy {:?} produced empty output", strategy);
    }
}
...
fn every_strategy_produces_valid_url_query() {
    ...
    for strategy in wafrift_encoding::all_strategies() {
        ...
        if let Ok(out) = result {
            ...
            assert!(!out.contains(' '), "strategy {:?} produced raw space in URL query", strategy);
        }
    }
}
