...
fn http() -> Client {
    ...
    fn build_post_json5_comment_emits_application_json_ct() {
        let h = http();
        let v = VECTORS.iter().find(|v| v.name == "POST-json5-comment").unwrap();
        let req = build_request_for_vector(v, &h, "http://x/", "q", "x", 0)
            .unwrap()
            .build()
            .unwrap();
    ...
    fn build_post_json5_comment_body_contains_block_comment() {
        ...
        assert!(body.contains("/*") && body.contains("*/"), "must carry a block comment: {body}");
    }
    ...
    fn build_post_ndjson_declares_ndjson_content_type() {
        ...
        let req = build_request_for_vector(v, &h, "http://x/", "q", "x", 0)
            ...
            .build()
            .unwrap();
        assert_eq!(req.headers().get("content-type").unwrap(), "application/x-ndjson");
    }
    ...
    fn build_post_json_as_form_declares_form_content_type() {
        let h = http();
        let v = VECTORS.iter().find(|v| v.name == "POST-json-as-form").unwrap();
        let req = build_request_for_vector(v, &h, "http://x/", "q", "x", 0)
            .unwrap()
            .build()
            .unwrap();
    ...
    fn build_post_json_as_form_body_lacks_form_kv_separator() {
        ...
        assert!(!body.contains("q="), "no form-shape KV pair in body: {body}");
    ...
    fn build_post_multipart_qp_declares_multipart_content_type() {
        let h = http();
        let v = VECTORS.iter().find(|v| v.name == "POST-multipart-qp").unwrap();
        let req = build_request_for_vector(v, &h, "http://x/", "q", "x", 0)
            .unwrap()
            .build()
            .unwrap();
    ...
    fn build_post_multipart_qp_unique_boundary_per_fire_counter() {
        let h = http();
        let v = VECTORS.iter().find(|v| v.name == "POST-multipart-qp").unwrap();
        let r0 = build_request_for_vector(v, &h, "http://x/", "q", "x", 0).unwrap().build().unwrap();
        let r1 = build_request_for_vector(v, &h, "http://x/", "q", "x", 1).unwrap().build().unwrap();
        let ct0 = r0.headers().get("content-type").unwrap().to_str().unwrap();
        let ct1 = r1.headers().get("content-type").unwrap().to_str().unwrap();
        assert_ne!(ct0, ct1, "boundary must differ across fires: {ct0} == {ct1}");
    }
