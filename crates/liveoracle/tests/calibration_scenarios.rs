...
fn calibrates_on_a_real_block_while_skipping_a_leading_reflection() {
    ...
    let reflected_xss = b(200, "you searched for <script>alert(1)</script> here", "<script>alert(1)</script>");
    ...
    assert_eq!(cal.classify(200, b"search results page footer nav"), Some(LiveVerdict::Allowed));
}
...
fn a_distinct_status_is_preferred_over_a_body_signal() {
    ...
    let blocked = b(403, "the application landing page content denied", "1' OR '1'='1");
    ...
    assert_eq!(cal.classify(403, b"unrelated body"), Some(LiveVerdict::Blocked));
    assert_eq!(cal.describe().to_lowercase().contains("status"), true, "describe: {}", cal.describe());
}

#[test]
fn a_body_discriminator_describes_a_block_page() {
    let benign = b(200, "results for your query item one item two item three", "c");
    let blocked = b(200, "access denied attack detected request id 4910 contact support", "1' OR '1'='1");
    let cal = calibrate(benign, vec![blocked]).expect("distinct 200 bodies calibrate");
    let d = cal.describe().to_lowercase();
    assert!(d.contains("block page") || d.contains("body"), "describe: {d}");
}
...
fn classify_returns_none_for_an_unfamiliar_status_under_a_status_discriminator() {
    ...
    assert_eq!(cal.classify(500, b"server error"), None, "an unknown status must defer");
}
...
fn all_reflected_controls_decline_calibration() {
    ...
    let controls = vec![
        b(200, "echo: <script>alert(1)</script>", "<script>alert(1)</script>"),
        b(200, "echo: 1' OR '1'='1", "1' OR '1'='1"),
        b(200, "echo: ../../etc/passwd", "../../etc/passwd"),
    ];
    assert!(calibrate(benign, controls).is_none(), "all-reflection target must decline");
}
...
fn the_malicious_control_battery_spans_multiple_attack_classes() {
    ...
    assert!(controls.iter().any(|c| c.contains("<script")), "needs an XSS control");
    assert!(controls.iter().any(|c| c.contains("OR '1'='1")), "needs a SQLi control");
    assert!(controls.iter().any(|c| c.contains("etc/passwd")), "needs a traversal control");
    assert!(controls.iter().any(|c| c.contains("cat ")), "needs a command-injection control");
}
